...
export async function POST(req: Request) {
  try {
    ...
    // accept both id and proofId (backwards compatible)
    ...
    // store evidence path + set pending
    db.prepare(
      `UPDATE proofs
       SET status = 'pending',
           evidencePath = ?,
    ...
    // ✅ IMPORTANT: redirect using origin (prevents localhost redirect on Render)
    const origin = req.headers.get("origin") || new URL(req.url).origin;
    return NextResponse.redirect(`${origin}/p/${id}`, 303);
  } catch (err: any) {
